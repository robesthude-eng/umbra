...
| Примечание | На проде БД без миграции 012 (`push_devices`); 013 (`calls.participants`) применена. Кто и чем катал серверные бинарии 8–10 сентября (v2, byusername, «0.9.1») — уточнить у владельца |
