# Журнал выкаток сервера Umbra

Сервер: `194.226.126.253`, systemd-юнит `umbra`, бинарник `/usr/local/bin/umbra-server`,
данные в `/opt/umbra` (PostgreSQL 16, блобы в `/opt/umbra/data/blobs`), `.env` — `/opt/umbra/.env`.
Внешний доступ: `http://194.226.126.253:8081` (nginx) и `https://game.agentwill.ru:8443`.

Автоматического журнала миграций нет, поэтому применённые миграции фиксируются здесь.

## 2026-09-11 — самообновление приложения (раздача /app/)

Клиент 0.15.0 (vc22) при запуске сверяет versionCode с `/app/latest.json`.
Серверная часть — только статика через nginx, Go-код не менялся.

| Параметр | Значение |
|---|---|
| Файлы | `/opt/umbra/app/latest.json` (версия+sha256+примечание) и `/opt/umbra/app/umbra-latest.apk` |
| nginx | `location = /app/latest.json` (json, no-cache) и `location /app/` (apk) добавлены во все три server-блока (80, 8081, 8443) |
| Бэкап vhost | `/root/umbra-deploy-bak/umbra.vhost.pre-app-update.<TS>` |
| Проверка | `latest.json` = 200 на 80/8081/8443 и снаружи по IP; APK = 200, `Content-Type: application/vnd.android.package-archive`, Content-Length совпадает |
| Текущая версия | versionCode 38 (0.16.15 — см. секцию выше), sha256 `066a032127042bb3a786e7a77d302ca4c032f81b305c48d9a1685a27fd1eee4a`; раздача обновлена 2026-09-12 08:41 MSK |

Порядок обновления при новых релизах: собрать APK в CI → скопировать в
`/opt/umbra/app/umbra-latest.apk` → перезаписать `latest.json` (versionCode,
versionName, sha256, notes). Клиенты при следующем запуске предложат обновление.

## 2026-09-12 — клиент 0.16.15 (vc38): снапшот «Настройки по категориям + Три темы»

Внешний снапшот, построен на нашем main (включает 0.16.12-мерж и OTA-патч
0.16.13 — проверено: IsoTime, независимый outbox, таймаут очереди, DiagLog,
сетевой монитор, onStart→onAppStart, UpdateCard в SettingsAbout.kt).
0.16.14: настройки по категориям. 0.16.15: три темы (Стандартная/Дымчатое
стекло/Alien), цвета оформления (6 палитр + обои Android 12+), миграция
выбора, 7 Android-тестов. versionCode 38 (37 занят авторской 0.16.14).

| Параметр | Значение |
|---|---|
| Коммиты | `e448492` (снапшот + роадмап 0.16.14/0.16.15) |
| CI | Оба воркфлоу зелёные на `e448492` (с первой попытки) |
| APK | sha256 `066a032127042bb3a786e7a77d302ca4c032f81b305c48d9a1685a27fd1eee4a`, 61 451 153 Б, versionCode 38 |
| Текущая версия | versionCode 38 (0.16.15), раздача обновлена 2026-09-12 08:41 MSK |

## 2026-09-11 — клиент 0.16.13 (vc36): надёжное самообновление

Инцидент: публикация vc35 не предлагалась телефону — проверка обновления
выполнялась один раз за холодный старт процесса, а Android держит процесс
неделями; «переоткрытие» без принудительной остановки проверку не запускало
(в логах — ноль запросов /app/latest.json от живого процесса).

| Параметр | Значение |
|---|---|
| Механика | Activity.onStart → AppUpdater.onAppStart: холодный старт и каждый возврат из фона; автокоoldown 15 мин; результат в AppUpdater.updateState; диалог из UmbraRoot; кнопка «Проверить обновление» (Настройки); ошибки в DiagLog (тег update-check) |
| Коммиты | `adb9364` (патч), `d38ca0c` (возврат потерянных импортов), `fce3e4c` (UpdateCard: вызов из SettingsTab + определение) — CI зелёный на `fce3e4c` |
| APK | sha256 `eebcf7dffa482f6aa309ba9c80ef9d415018380d80a0b4bfe6fce7e69c98c33c`, 61 402 005 Б, versionCode 36 |
| Текущая версия | versionCode 36 (0.16.13), раздача обновлена 2026-09-12 00:11 MSK |

## 2026-09-11 — клиент 0.16.12 (vc35): мерж двух параллельных веток 0.16.11

Автор выпустил две параллельные 0.16.11 (обе vc34): «Красивые фото и свайпы»
(интегрирована ранее как c59e0c8/vc34) и «Дымчатое стекло» (отдельный снапшот,
база 0.16.10, без фото/свайпов). Объединены 3-way мержем (база b1cc429):
временный коммит стекла d39b87d → merge в main; авто-мерж ChatView/MainShell/
ChatKit прошёл сам, конфликты AuthFlow/CallScreen разрешены совмещением
(стекло > Alien-градиент > стандартный вид).

| Параметр | Значение |
|---|---|
| Коммиты | `d39b87d` (tmp-коммит стекла), `a6eb8ab` (мерж = 0.16.12 vc35) |
| CI | Оба воркфлоу зелёные на `a6eb8ab` |
| APK | sha256 `38148b7de9975a1fb515a98e1d498a5f95344841b8d54a192265f8bc0e81017e`, 61 402 001 Б, versionCode 35 |
| Текущая версия | versionCode 35 (0.16.12), раздача обновлена 2026-09-11 23:32 MSK |
| Урок | Если снапшот-версия уже выпущена (совпадает versionName/vc), следующая получает новый versionCode — иначе самообновление не увидит её. Параллельные снапшоты проверять на пересечение по базе (git merge, не cp) |

## 2026-09-11 — клиент 0.16.11 (vc34): снапшот «Красивые фото и свайпы»

Внешний снапшот (Drive). Сетевые фиксы 0.16.5–0.16.8 подтверждены до интеграции
(IsoTime, независимый outbox, таймаут 30 с, DiagLog, инкрементальный refresh,
сетевой монитор). Снапшот основан на коде автора до CI-фиксов 0.16.10 — оба
фикса применены повторно в коммите снапшота; роадмап: восстановлена секция
0.16.10 (затёрта снапшотом), добавлена 0.16.11.

| Параметр | Значение |
|---|---|
| Коммиты | `c59e0c8` (снапшот + повторные фиксы импортов remember/weight + роадмап) |
| CI | Оба воркфлоу зелёные на `c59e0c8` (с первой попытки) |
| APK | sha256 `957e18b73b830b9a3e463da32da6b6e71948c324dcae832c9b390dfaf6c74d06`, 61 385 617 Б, versionCode 34 |
| Текущая версия | versionCode 34 (0.16.11), раздача обновлена 2026-09-11 22:28 MSK |

## 2026-09-11 — клиент 0.16.10 (vc33): снапшот Alien Interface II

Внешний снапшот (Drive, без корневой папки). Сетевые фиксы 0.16.5–0.16.8 подтверждены
в коде снапшота до интеграции: IsoTime (3 места, 0 сырых Instant.parse), независимый
outbox, таймаут 30 с + счётчик попыток, DiagLog + wiring, инкрементальный refresh,
«Медленный режим», сетевой монитор ConnectivityManager. Новое: Alien Interface II —
три интенсивности (`alien_intensity`), AlienVisuals.kt, HUD, стилизация переписки.

| Параметр | Значение |
|---|---|
| Коммиты | `b1cc429` (снапшот), `58fb34c` (импорт remember в ChatKit + роадмап 0.16.10), `2c8288b` (убрать illegal-импорт layout.weight в AlienVisuals) |
| CI | Оба воркфлоу зелёные на `2c8288b` |
| APK | sha256 `12e88ec450a7177b1715dc07680404e4a4b8db8371b927dac517e5b84cd5dad5`, 61 336 469 Б, versionCode 33 |
| Текущая версия | versionCode 33 (0.16.10), раздача обновлена 2026-09-11 20:48 MSK |

## 2026-09-11 — клиент 0.16.9 (vc32): снапшот 0.16.7–0.16.9 — Network Recovery, Motion, Alien Interface

Внешний снапшот (Drive), пачка трёх версий. Сетевые фиксы 0.16.5–0.16.6 подтверждены
в коде снапшота до интеграции: IsoTime-парсер (3 места, 0 сырых Instant.parse),
флаш outbox независимо от синхронизации, таймаут мьютекса 30 с + счётчик попыток,
DiagLog + wiring, инкрементальный refresh при переподключении, «Медленный режим».
Новое: монитор ConnectivityManager с пересозданием WS при смене маршрута,
jitter в backoff, встроенная проверка сети в настройках, Alien Interface (opt-in).

| Параметр | Значение |
|---|---|
| Коммиты | `6bf09f1` (снапшот), `1c0e9a0` (фикс импортов: ExperimentalActivityApi убран — PredictiveBackHandler стабилен; Color/RoundedCornerShape в SettingsTab) |
| CI | Оба воркфлоу зелёные на `1c0e9a0` |
| APK | sha256 `2c745d4cafa3c1455743722016c009430ba6b6405a3103c7eb77f29268cf3817`, 61 303 701 Б, versionCode 32 |
| latest.json | Путь `/app/umbra-latest.apk` (со слэшем — после инцидента с 0.16.6) |
| Текущая версия | versionCode 32 (0.16.9), раздача обновлена 2026-09-11 18:16 MSK |

## 2026-09-11 — хотфикс latest.json: путь к APK без ведущего слэша ломал скачивание 0.16.6

После выкладки 0.16.6 тест-телефон четыре раза проверил latest.json, но скачивание
падало мгновенно без единого запроса к APK: в json был `"apk": "umbra-latest.apk"`
(без `/`), а клиент строит URL как `base + apk` → `http://194.226.126.253:8081umbra-latest.apk`
— недопустимый адрес, IllegalArgumentException до HTTP. В json для vc28 путь был
корректный (`/app/umbra-latest.apk`), поэтому предыдущие обновления работали.

| Параметр | Значение |
|---|---|
| Фикс | `"apk": "/app/umbra-latest.apk"` в `/opt/umbra/app/latest.json`; бэкап `/root/umbra-deploy-bak/latest.json.pre-apk-path-fix.20260911` |
| Проверка | curl снаружи: json 200 с корректным путём; APK 206 на range-запрос |
| На будущее | В клиенте (план 0.16.7): терпимый path-join в AppUpdater.download + DiagLog ошибок обновления; в latest.json всегда путь от корня со слэшем |

## 2026-09-11 — клиент 0.16.6 (vc29): починка синхронизации/отправки после рестарта сервера в TZ Europe/Moscow

Инцидент «test-устройство принимает, но не отправляет»: 2026-09-10 ~09:36/20:44
на прод был накатан пересобранный серверный бинарий и сервис перезапущен в зоне
Europe/Moscow — pgx декодирует timestamptz в локальную зону процесса, поэтому
все метки из БД в `GET /v1/messages` получили смещение `+03:00`, а `Instant.parse`
на Android понимает только `Z`: синхронизация падала на первом сообщении,
а `flushOutbox` выполнялся только после успешной загрузки истории — очередь
отправки не уходила вовсе. Приём работал через живой WebSocket (метки там
из `time.Now().UTC()`, суффикс `Z`).

| Параметр | Значение |
|---|---|
| Клиент | 0.16.6 (vc29): толерантный парсер `IsoTime`, флаш очереди не зависит от синхронизации |
| Коммиты | `fa3bb82` (клиент), `79e0efb` (сервер: `.UTC()` на всех DB-метках — в прод пока НЕ деплоился) |
| APK | `/opt/umbra/app/umbra-latest.apk`, sha256 `6cffa3a25f860e6664e2f59671e42457e62f91ac73421aaaa184a5ef5e99286d`, 61 254 517 Б |
| Сервер | Решение владельца: TZ=UTC + рестарт. Выполнено 2026-09-11 16:53:08 MSK: drop-in `/etc/systemd/system/umbra.service.d/tz.conf` (`Environment=TZ=UTC`), `daemon-reload` + рестарт; бэкап юнита `/root/umbra-deploy-bak/umbra.service.pre-tzutc.<TS>`; проверено: TZ=UTC в `/proc/<pid>/environ`, `/healthz` ok, PostgreSQL/Telegram-бот подняты, логи процесса идут в UTC. Инкрементальный фикс `.UTC()` (79e0efb) остаётся в main на будущие выкатки |
| Примечание | Все серверные выкатки 8–10 сентября (v2, byusername, «0.9.1») выполнялись в сессиях arena.ai; текущий бинарий — сборка main (маршруты /v1/channels, /v1/by-username подтверждены), перезапуск 10.09 20:44 в зоне Europe/Moscow и включил баг «+03:00». На проде БД без миграции 012 (`push_devices`); 013 (`calls.participants`) применена |

## 0.9.1 — исправления звонков и медиа (фикс-пак)


| Параметр | Значение |
|---|---|
| Дата | 2026-09-10 |
| Коммит | `c33712a` (vc14, CI зелёный: checks + APK) |
| Бинарник | sha256 `3bb77be757879db84cd8308225903f0cfea475a03c1ce842b12041d9ae3ac88d` |
| Бинарник gc | sha256 `9fb35b6c6a458b0d869850e897f1e9615826de2cd440dfc6048020183ca8ade3` (обновлён заодно: прежний остался от старой сборки; dry-run чист) |
| Бэкап БД | `/root/umbra-deploy-bak/umbra-pre0.9.1.20260910-204351.dump` (pg_dump -Fc от postgres, 14 TABLE DATA) |
| Бэкапы | `umbra-server.pre-0.9.1.20260910-204351`, `umbra-gc.pre-0.9.1.20260910-204351`, `umbra.env.pre-0.9.1.20260910-204351` |
| Миграции | не требуются (новых нет); переменные окружения не менялись |
| Проверка | `/healthz` = 200 ×3, `/v1/turn` = 401, `/v1/calls` = 401, `/v1/media` = 405, `/v1/push/devices` = 405, через nginx `:8081` = 401; TURN по-прежнему выдаёт учётки |
| Данные | users=1, media=2, messages=0, chats=0, calls=0 (не изменились) |

Суть серверных правок 0.9.1: выход одного участника группового звонка не завершает
разговор в базе (`callLeaverStore` в памяти процесса, TTL 6 ч) — запись закрывается
только при отмене звонящим до ответа или когда остался ≤1 участник. Перезапуск
сервера посреди разговора возвращает старое поведение для текущих звонков (документировано
в `FIXES_SCREENSHOTS.md`), данные не теряются.

Откат: вернуть `umbra-server.pre-0.9.1.<TS>` и перезапустить юнит.

## 0.9.0 — вложения и групповые звонки

| Параметр | Значение |
|---|---|
| Дата | 2026-09-10 |
| Бинарник | sha256 `d818a9fe2ec6171ddb4013bb9b0cff1bd52d423d852de1f5821edbf9a86a5363` |
| Бэкап БД | `/root/umbra-deploy-bak/umbra-pre0.9.0.20260910-060940.dump` (pg_dump -Fc, 28 объектов) |
| Бэкап бинарника | `/root/umbra-deploy-bak/umbra-server.pre-0.9.0.20260910-060940` |
| Бэкап env | `/root/umbra-deploy-bak/umbra.env.pre-0.9.0.20260910-060940` |
| Миграция | `013_call_participants.sql` применена однократно (`ALTER TABLE` / `UPDATE 0` / `CREATE INDEX`) |
| Проверка | `/healthz` = 200, `/v1/turn` = 401, `/v1/calls` = 401, `/v1/push/devices` = 405 |
| Данные | users=1, media=2, messages=0, chats=0 (не изменились) |

Новых переменных окружения для 0.9.0 не требуется. TURN и FCM по-прежнему не
настроены: `/v1/turn` отвечает отказом без учётки, клиент использует STUN.

## 2026-09-10 — coturn (TURN) и лимит вложений nginx

Выполнено по решению владельца (пункты «поднять TURN» и «поднять лимит nginx»),
без изменения кода: сервер 0.9.0 и APK versionCode 13 уже поддерживают TURN
через `GET /v1/turn` (временные учётки TURN REST API).

### coturn (пакет `coturn` 4.6.1, юнит `coturn`)

| Параметр | Значение |
|---|---|
| Порты | слушает 3478/udp+tcp на 192.168.0.7; relay 49152–49251/udp+tcp |
| NAT | сервер за NAT хостера (внутр. 192.168.0.7 ↔ публ. 194.226.126.253), задан `external-ip=194.226.126.253` |
| Авторизация | `lt-cred-mech` + `use-auth-secret`, секрет в `TURN_SECRET` в `/opt/umbra/.env` (в репозиторий не выкладывать) |
| realm | `agentwill.ru` |
| ufw | добавлены правила `3478/tcp`, `3478/udp`, `49152:49251/udp`, `49152:49251/tcp` |
| `.env` umbra | `TURN_URL=turn:194.226.126.253:3478?transport=udp,turn:194.226.126.253:3478?transport=tcp`, `TURN_SECRET=<тот же секрет>`, `TURN_TTL_SECONDS=3600`; сервис перезапущен |
| Логи | `/var/log/turnserver.log` (simple-log), ротация `/etc/logrotate.d/turnserver` (weekly ×4, copytruncate) |
| Бэкапы | `/root/umbra-deploy-bak/turnserver.conf.orig-apt` (конфиг пакета), `umbra.env.pre-turn.20260910-093614` |

Особенности конфигурации: закрыты приватные диапазоны через `denied-peer-ip`,
но `allowed-peer-ip=192.168.0.7` обязателен — `external-ip` отражает адреса
peers во внутреннюю форму, и без этого исключения relay↔relay (двое звонящих
через один TURN) получает 403 Forbidden IP на CHANNEL_BIND. TLS/DTLS на TURN
не включены (сертификата нет; UDP/TCP 3478 достаточно для реле).

Проверено с внешнего адреса (не с сервера): аллокации по временной учётке
(REST-подпись совпадает с выдачей `/v1/turn`), STUN Binding по UDP,
TCP-подключение к 3478, обмен данными relay↔relay и relay(TCP)↔relay(UDP)
в обе стороны; peer-адреса клиент видит в публичной форме (hairpin за NAT
обрабатывается корректно). `GET /v1/turn` возвращает STUN Google + два TURN URL
с учёткой TTL 3600.

### nginx: лимит вложений

Во всех трёх блоках vhost `umbra` (80, 8081, 8443) `client_max_body_size`
поднят `64m → 256m` — теперь соответствует `MAX_USER_MEDIA_BYTES=268435456`
(256 МиБ). `nginx -t` ок, reload без простоя. Проверка: POST 70 МБ и 250 МБ
на `/v1/media` через `:8081` — HTTP 401 (до правки 70 МБ давал 413 от nginx;
401 = тело пропущено nginx до приложения, отказ по авторизации).


## 0.4.2 — безопасность сессий

| Параметр | Значение |
|---|---|
| Дата | 2026-09-09 |
| Бинарник | sha256 `dc91d0e67dc0c8b6c1444e985d8ac462dc23fd50e71107a7fb716d942a388131` |
| Бэкап БД | `/root/umbra-deploy-bak/umbra-pre0.4.2.20260909-132503.dump` |
| Бэкап бинарника | `/root/umbra-deploy-bak/umbra-server.pre-0.4.2.20260909-132503` |
| Миграция | `011_revoke_legacy_sessions.sql` применена однократно (отозван 1 токен) |
| Env | `TOKEN_TTL_SECONDS=2592000`, `ALLOW_LEGACY_AUTH=false`, `TRUSTED_PROXIES=127.0.0.1,::1` |

## Открытые вопросы по инфраструктуре

- **HTTPS на домене не работает для Umbra.** A-запись `api.agentwill.ru` ведёт на
  сервер, но vhost Umbra слушает только порт 80; на 443 запросы с этим SNI попадают
  в чужой vhost (`chassis.agentwill.ru ... _`) и получают HTML другого проекта.
  Рабочие адреса API: `http://194.226.126.253:8081` и `https://game.agentwill.ru:8443`.
  Чтобы включить HTTPS на домене, нужен сертификат для `api.agentwill.ru` и
  `listen 443` в vhost `umbra` (правки конфигов `web-game`/`chassis-3d` не нужны,
  но порт 443 общий — решение за владельцем сервера).
- **TURN поднят 2026-09-10** (coturn, см. раздел выше) — пункт закрыт.
- **FCM не настроен.** Без `google-services.json` сборка APK идёт без push.
- **Лимит nginx** `client_max_body_size` поднят до `256m` 2026-09-10 под квоту
  `MAX_USER_MEDIA_BYTES` (256 МиБ) — пункт закрыт.

## 2026-09-12 — 0.16.16 (vc39): медиа по области видимости + честные звонки

**Бинаррь:** CI run `34679445412` (воркфлоу server-binary.yml, commit `e7b35f7`), sha256 `f80789fe24c486ffafe6e471989803b9c1d2dc429f14b1c09bed9ccfe44fbf92`, 13 332 640 Б, static linux-amd64. Приложен как `/root/umbra-server-0.16.16`, установлен в `/usr/local/bin/umbra-server` (10:03 UTC+3).

**Бэкапы** (`/root/umbra-deploy-bak/`): pg_dump umbra, предыдущий бинаррь, `.env`.

**Миграции** (через `cat … | sudo -u postgres psql umbra -v ON_ERROR_STOP=1`, т.к. postgres не читает /root):
- `012_push_devices.sql` — таблица push_devices (задел под FCM, отложено пользователем);
- `014_media_scope.sql` — media.chat_id/recipient_id (nullable) + частичные индексы.

**.env:** `MEDIA_LEGACY_OPEN_ACCESS=1` — легаси-файлы (без scope) доступны всем авторизованным, чтобы старые вложения в переписках не пропали. Новые загрузки всегда пишут scope.

**Проверки после рестарта:** active, TZ=UTC (drop-in), healthz `{"status":"ok"}`, PostgreSQL + blob-хранилище + Telegram-бот в логе, `/v1/media/*` без токена → 401, `/app/latest.json` → 200, APK → 206.

**Клиент:** vc39 (0.16.16, sha256 `9a83e73d…`) опубликован на /app/ 10:03 UTC+3 (61 451 153 Б… см. latest.json). Порядок «сервер → клиент» соблюдён (старый сервер давал бы 400 на multipart-поля chat_id/recipient_id).

**Текущая версия:** сервер 0.16.16 (vc39), клиент 0.16.16 (vc39).

## 2026-09-14 — 0.16.18 (vc41): шифрование хранилища + защита аккаунтов

**Бинарни:** CI run `34837430024` (server-binary.yml, `f65631f`): `umbra-server` sha256 `7734323e49fd…` (13 521 056 Б), CLI `umbra-storage` sha256 `640519555fb2…` (12 382 368 Б). Установлены в `/usr/local/bin/` (14:31 MSK).

**Бэкапы** (`/root/umbra-deploy-bak/`, TS=20260914-142955): pg_dump umbra, бинаррь, `.env`, **tar каталога blobs** (35 МБ) и **копия ключевого файла** storage-keys (0600).

**Миграции** (stdin-пайп под postgres):
- `015_cloud_storage.sql` — media.storage_format/blob_id, таблица storage_keys;
- `016_account_security.sql` — auth_tokens.session_id/created_at/last_seen_at/device_name, лимит 90 дней, push_devices.session_id.

**Инцидент при деплое (исправлен):** сервер падал с `permission denied for table push_devices` — таблицы `push_devices` (012) и `storage_keys` (015) были созданы psql-миграциями от postgres и принадлежали postgres, а не роли сервера `umbra`. Исправлено: `ALTER TABLE … OWNER TO umbra`. Урок: после psql-миграций, создающих таблицы, сверять tableowner. Даунтайм ~1 мин (crash-loop до фикша).

**Ключи шифрования:** `/opt/umbra/data/storage-keys.json` (0600, владелец umbra), создан `umbra-storage init`; `STORAGE_KEY_FILE` прописан в `.env`. Копия ключа — в бэкапе; **при потере файла новые зашифрованные данные не восстановить** (легаси-формат 0 не затронут).

**Совместимость:** `AUTH_ALLOWED_PHONES` не задан → вход разрешён существующим пользователям (новые регистрации требуют allowlist). Легаси-сессии сохранены (2), клиенты vc39 работают: вход по прежней OTP-схеме (без device_name → legacy-режим), но удаление аккаунта недоступно (нужен отдельный код из vc41).

**Проверки после рестарта:** active, лог «используется PostgreSQL с шифрованием облачного хранения», TZ=UTC, healthz ok, media без токена 401, /app/ 200, storage_keys=1, сессии=2, легаси-медиа=10 (формат 0).

**Клиент:** APK vc41 (sha256 `95fc80cb…`, 61 483 893 Б) опубликован на /app/ 14:32 MSK (latest.json 41/0.16.18, HTTP 206).

**Отложено:** `umbra-storage migrate` (дошифровка старых сообщений/вложений) — не выполнялся, старые данные остаются в формате 0; выполнить позже по желанию владельца.

**Текущая версия:** сервер 0.16.18 (vc41), клиент 0.16.18 (vc41).

## 2026-09-14 — 0.17.1 (vc42): presence-сервер + клиент 0.17.0/0.17.1

**Бинаррь:** CI run `34848938691` (server-binary.yml, `18efd25`), `umbra-server` sha256 `e00def23bb45…` (13 537 440 Б). Установлен в `/usr/local/bin/umbra-server` (16:30 MSK).

**Бэкапы** (`/root/umbra-deploy-bak/`, TS=20260914-162937): pg_dump, бинаррь, `.env`. Blobs не копировались — миграция их не затрагивает.

**Миграция 017** (user_presence): `users.last_seen_at` + `users.hide_last_seen`. Только ALTER существующей таблицы → проблем с владельцем нет (урок 0.16.18 учтён).

**Серверные эндпоинты:** `GET /v1/users/{id}/presence`, `POST /v1/account/privacy`; touchPresence в requireAuth с дебаунсом 30 с. Остальное (typing-роуты) существовало раньше — клиент 0.17.1 лишь подключил их.

**Интеграция снапшота:** автор не собирал проект; CI поймал 6 ошибок (несуществующий API media3 1.4.1: createForShortSide, Effects не в том пакете, layout-параметр; пропущенный импорт ChatMedia; задвоенный @Composable; счётчик миграций 16→17 ×3). Исправлено в `03b0666`, `a16f906`, `18efd25`. API media3 сверялся по артефактам dl.google.com.

**Проверки после рестарта:** active (с первого раза), шифрование хранилища активно, healthz ok, presence/privacy без токена 401, /app/ 200.

**Клиент:** APK vc42 (sha256 `18b50e41…`, 64 312 177 Б) опубликован на /app/ 16:31 MSK (latest.json 42/0.17.1, HTTP 206).

**Текущая версия:** сервер 0.17.1, клиент 0.17.1 (vc42).

## 2026-09-14 — 0.18.0 (vc46): прочтения в группах + предпросмотр ссылок + OutboxWorker

**Бинаррь:** CI run `34861939468` (server-binary.yml, `ade08ee`), `umbra-server` sha256 `a9fed48d8847…`. Установлен в `/usr/local/bin/umbra-server` (18:27 MSK).

**Бэкапы** (`/root/umbra-deploy-bak/`, TS=20260914-182656): pg_dump, бинаррь, `.env`.

**Миграции:** 018 (`message_reads` — курсор прочтения на пару), 019 (`chat_reads` — курсор на участника группы). Обе создают таблицы → сразу выполнено `ALTER TABLE … OWNER TO umbra` (урок 0.16.18 применён, рестарт прошёл с первого раза).

**Серверные эндпоинты:** унифицированные `POST/GET /v1/chats/{id}/read` (ЛС и группы), `GET /v1/link-preview` — сервер сам забирает og-мета; SSRF-защита в DialContext (IP проверяется после резолва — DNS rebinding закрыт; loopback/private/CGNAT/169.254.169.254 заблокированы; ≤3 редиректов, 6 с, 512 КБ, кэш 6 ч с отрицательными результатами).

**Интеграция снапшота:** снова база до наших фиксов — восстановлены VideoCompressor/ChatMedia, убраны задвоенный `@Composable` (ChatView) и дубликат `import retrofit2.Response`; журнал роадмапа 0.17.0–0.18.0; счётчики миграций 16→19 (×3). Коммиты `684d314`, `ade08ee`; CI зелёный, APK vc46 собран.

**Проверки после рестарта:** active с первого раза, шифрование хранилища активно, healthz ok, read/link-preview без токена 401, /app/ 200.

**Клиент:** APK vc46 (sha256 `13149246…`, 64 559 821 Б) опубликован на /app/ 18:27 MSK (latest.json 46/0.18.0, HTTP 206).

**Текущая версия:** сервер 0.18.0, клиент 0.18.0 (vc46).

## 2026-09-14 — 0.18.3 (vc47): payload-алиас, докачка обновлений, фиксы

**Бинаррь:** CI run `34868772842` (server-binary.yml, `82df513`), `umbra-server` sha256 `fc186dcb4e9f…`. Установлен 19:30 MSK. **Миграций нет.**

**Сервер:** API-поле тела сообщения `ciphertext` → `payload` (алиас совместим в обе стороны: клиент шлёт оба поля, сервер принимает оба; тест payload_alias_test). Документация модели доверия переписана честно: E2EE нет, сервер видит base64-тело и шифрует его только при записи.

**Клиент:** APK vc47 (sha256 `eff2c71c…`) опубликован на /app/ 19:30 MSK. Обновление APK: докачка Range/206, кэш целого файла, чистка старых версий; docs/UPDATE_GUIDE.md — инструкция с фотографиями; StyleTokens-рефакторинг темы.

**CI:** сборка APK теперь требует HTTPS serverUrl (http — осознанный opt-in через input allowInsecureServerUrl или переменную репозитория UMBRA_ALLOW_INSECURE). Заданы переменные репозитория: UMBRA_SERVER_URL=http://194.226.126.253:8081, UMBRA_ALLOW_INSECURE=true — до перевода прода на https://api.agentwill.ru; после перевода переменные поменять, флаг снять.

**Интеграция:** versionCode 46→47 (в снапшоте не поднят — фиксы не дошли бы по OTA); в третий раз восстановлены VideoCompressor/ChatMedia-импорт/задвоенный @Composable/дубликат import Response (регрессии снапшота автора); счётчики миграций 16→19; mojibake ×4 вычищен; журнал роадмапа восстановлен + строка 0.18.3; CHANGES переехал в docs/history/.

**Проверки после рестарта:** active, healthz ok, media/presence живы, /app/ 200/206.

**Текущая версия:** сервер 0.18.3, клиент 0.18.3 (vc47).

## 2026-09-15 — сервер 0.18.5-hotfix: Telegram-поллинг без ошибок, чистка диска

**Бинаррь:** CI run `34908276484` (server-binary.yml, `2cf3c4e`), `umbra-server` sha256 `da85b4e6279d…`. Установлен 02:20 MSK 15.09. Миграций нет. Откат: `/root/umbra-server.pre-tgh11-v2-20260914-232017`.

**Проблема:** `getUpdates` через Worker-релей падал каждые ~45 секунд — «stream error: PROTOCOL_ERROR» (HTTP/2), после перехода на HTTP/1.1 — «context deadline exceeded» раз в ~2 минуты. Диагностика: дефект края Cloudflare на переиспользуемых соединениях к workers.dev — на одном keep-alive соединении 8 зависаний на 178 запросов, на свежих соединениях 0 на 100. Для h2 зависание проявлялось RST_STREAM(PROTOCOL_ERROR), для h1.1 — молчанием до таймаута клиента.

**Серверные изменения (`internal/telegram/client.go`):** принудительный HTTP/1.1 (TLSNextProto без h2), `DisableKeepAlives` — свежее соединение на каждый запрос, таймаут клиента 30с (было 60с). Тесты `internal/telegram/client_test.go`: отключённый h2, свежие соединения, фактический протокол запроса (h1.1).

**Воркер:** в `umbra-tg` добавлен 10-секундный `AbortSignal` на upstream fetch к api.telegram.org — зависания апстрима больше не держат соединение (изменение на стороне Cloudflare, вне репозитория; исходник — в резервных скриптах).

**Результат:** до фикса — 1-2 ошибки в минуту; после — 0 ошибок за 5 минут наблюдения (~300 запросов поллинга). Стресс-тест релея со свежими соединениями: 100/100, max latency 3.2s. Ресурс: RSS 18 МБ, service active.

**Инфраструктура:** переменная репозитория `UMBRA_SERVER_URL` задана (`https://game.agentwill.ru:8443`, сертификат до 15.11.2026) — блокер релиза закрыт. Диск очищен 85% → 70% (удалены 12 старых APK, 7 старых бинарников, umbra-deploy-bak 389 МБ, журнал вакуумирован −204 МБ; сохранены umbra-0.18.3/0.18.5.apk, umbra-server-0.18.3/0.18.5, конфиг-бэкапы в /root/umbra-env-backup).

**Текущая версия:** сервер 0.18.5 + telegram-hotfix, клиент 0.18.5 (vc48).

## 2026-09-14 — 0.18.5 (vc48): доступность, формы тем, conformance-тест, распил ChatView

**Бинаррь:** CI run `34888052978` (server-binary.yml, `5b251ae`), `umbra-server` sha256 `1cb43bf2731e…`. Установлен 22:39 MSK. **Миграций нет.**

**Серверные изменения:** conformance-тест (444 строки, 15 групп кейсов на MemoryStore+PostgresStore, миграции по glob) вскрыл расхождение семантики read-курсоров: Memory возвращал ErrNotFound для несуществующего читателя/чата, Postgres — (zero, nil). Postgres приведён к единой семантике (postgres_reads.go): несуществующая сущность → ErrNotFound (404), «ещё не читал» → нулевое время. Для существующих собеседников поведение не изменилось.

**Клиент:** APK vc48 (sha256 `909ba363…`, 64 592 589 Б) на /app/ 22:39 MSK. 0.18.4: тач-таргеты 48 dp, TalkBack-contentDescription, ShapeKit — формы полей/пузырей/чипов из темы. 0.18.5: экран чата разобран (ChatView 1772→875 + ChatComposerSection/ChatMessageRow/ChatDialogs, типы в internal).

**Интеграция:** versionCode 46→48 (у автора не поднят); регрессии авторской базы восстановлены (VideoCompressor, импорт ChatMedia, vars-фикс CI, счётчики 19, mojibake ×4, дубликат import Response); CI-фиксы: импорт Shape (ChatKit), семантика Postgres; CHANGES-секции 0.18.3/0.18.4 возвращены, строки журнала роадмапа добавлены; удалены 17 корневых дублей доков (оставались после переезда в docs/). Один ранн упал по инфраструктурной причине (битый zip Android Emulator на раннере) — перезапущен, success.

**Проверки после рестарта:** active с первого раза, healthz ok, read-роут 401 без токена, /app/ 200/206.

**Текущая версия:** сервер 0.18.5, клиент 0.18.5 (vc48).
