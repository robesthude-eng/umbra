...
| Область | Реализация |
|---|---|
| Аккаунт | Username + Ed25519 challenge-response, отзыв токена при выходе |
| Личная переписка Android | Signal, постоянная очередь отправки, локальная зашифрованная история |
...
| Таймер | API expires_in, удаление истёкших сообщений на сервере и Android; UI настройки таймера пока нет |
...
На новом томе выполняются все шесть миграций. Для более ранней схемы сначала
