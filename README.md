...
- Секретные сообщения с самоуничтожением (`expires_in`): сервер не отдаёт просроченные.
- «Сжечь аккаунт» — полное необратимое удаление пользователя и всех его данных.
- Хранилище блобов: файловое (по умолчанию) или S3-совместимое (AWS S3 / MinIO); квота медиа на пользователя и GC осиротевших блобов.
...
| `MAX_USER_MEDIA_BYTES` | `0` (без лимита) | Квота суммарного объёма медиа на пользователя |
| `BLOB_STORE_TYPE` | `file` | `file` или `s3` |
| `S3_ENDPOINT` / `S3_ACCESS_KEY` / `S3_SECRET_KEY` / `S3_BUCKET` / `S3_REGION` / `S3_USE_SSL` | — | Настройки S3/MinIO при `BLOB_STORE_TYPE=s3` |

При `BLOB_STORE_TYPE=s3` блобы хранятся в S3-совместимом объектном хранилище
(AWS S3, MinIO, Backblaze B2 и др.). Бакет создаётся автоматически при старте.

### GC осиротевших блобов

После аварийного завершения между записью blob и сохранением метаданных могут остаться
«сироты». Удалить их:

```bash
BLOB_DIR=./data/blobs go run ./cmd/gc        # файловое хранилище
STORE=postgres DATABASE_URL=... BLOB_STORE_TYPE=s3 S3_ENDPOINT=... S3_BUCKET=... go run ./cmd/gc
```
...
| GET | `/v1/account` | информация о текущем аккаунте |
| POST | `/v1/account/burn` | полностью удалить аккаунт (необратимо) |
...
Против прод-сервера (лимит 50 MiB по умолчанию) добавьте `--timeout` — дефолтные
20 секунд curl могут не хватить на загрузку файла точного лимита по медленной сети:

```bash
python3 scripts/smoke-media.py --base-url https://ДОМЕН --limit 52428800 --timeout 120
```
...
Файловый backend рассчитан на один сервер или общее файловое хранилище; для кластера
используйте S3/MinIO (`BLOB_STORE_TYPE=s3`). Квоты пользователей (`MAX_USER_MEDIA_BYTES`)
и GC осиротевших блобов (`cmd/gc`, льготный период `-min-age` — защита от удаления
незавершённых загрузок) реализованы; расписание — `deploy/umbra-gc.timer`,
бэкапы — `deploy/umbra-backup.timer` (см. `docs/deploy.md` §6).
...
- [x] Этап 5, сервер: секретные сообщения (самоуничтожение) и «сжечь аккаунт»
- [x] Backend S3/MinIO, квота медиа, GC осиротевших блобов
- [ ] Медиа на Android-клиенте
