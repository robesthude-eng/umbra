...
func TestEd25519RawAndLegacyDER(t *testing.T) {
	...
	for _, key := range [][]byte{pub, der} {
		...
		if err != nil || !bytes.Equal(raw, pub) {
			t.Fatalf("normalize: %x %v", raw, err)
		}
		if !VerifyEd25519(key, msg, sig) {
			t.Fatal("valid signature rejected")
		}
		if VerifyEd25519(key, msg, sig[:63]) {
			t.Fatal("short signature accepted")
		}
		if VerifyEd25519(key, []byte("other"), sig) {
			t.Fatal("wrong message accepted")
		}
	}
	if _, err := NormalizeEd25519(der[:len(der)-1]); err == nil {
