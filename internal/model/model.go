...
//
// МОДЕЛЬ ДОВЕРИЯ (без прикрас): сквозного шифрования в текущей версии НЕТ.
// Клиент присылает тело сообщения как base64(utf8(JSON)) — это кодирование,
// а не шифрование, и сервер видит содержимое. Сервер шифрует его только при
// записи в базу и в блоб-сторедж своими ключами (см. internal/cloudcrypto).
// Приватных ключей пользователей на сервере нет.
//
// Поля pre-key (IdentityX25519, SignedPrekey, OneTimePrekeys) зарезервированы под
// будущий E2EE и сегодня СОДЕРЖИМОЕ СООБЩЕНИЙ НЕ ЗАЩИЩАЮТ.
package model
...
// заполнен ChatID (RecipientID при этом пуст).
//
// Ciphertext — историческое и вводящее в заблуждение имя: внутри лежит НЕ
// шифротекст клиента, а base64 от JSON-конверта. В HTTP API каноническое имя
// этого поля — payload; ciphertext отдаётся и принимается как deprecated-алиас
// для клиентов до 0.19. Сервер шифрует содержимое при записи и расшифровывает
// при выдаче авторизованному устройству, то есть видит текст.
//
