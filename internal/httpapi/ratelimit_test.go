...
func TestProxyClientAddress(t *testing.T) {
	...
	for _, tc := range []struct { name, peer, forwarded, want string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := l.clientIP(r); got != tc.want { t.Fatalf("got %q, want %q", got, tc.want) }
		})
	}
}
...
func TestProxyUsersHaveSeparateOTPLimits(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		if code := request("127.0.0.1:1234", "203.0.113.1"); code != 204 { t.Fatal(code) }
	}
	if code := request("127.0.0.1:1234", "203.0.113.1"); code != 429 { t.Fatal("limit not enforced") }
	if code := request("127.0.0.1:1234", "203.0.113.2"); code != 204 { t.Fatal("users share proxy limit") }
	for i := 0; i < 6; i++ {
		if code := request("198.51.100.1:1234", "203.0.113.1"); code != 204 { t.Fatal(code) }
	}
	if code := request("198.51.100.1:1234", "203.0.113.2"); code != 429 { t.Fatal("spoofed header bypassed direct limit") }
}
