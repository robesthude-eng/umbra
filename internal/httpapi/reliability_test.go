...
func TestRegisterLegacyDERAndLogout(t *testing.T) {
	...
	if code, body := register(t, h, kb); code != 201 { t.Fatalf("register DER: %d %v", code, body) }
	token := authenticate(t, h, "legacy", kb.privEd)
	if code, _ := doReq(t, h, "POST", "/v1/auth/logout", nil, token); code != 200 { t.Fatalf("logout: %d", code) }
	if code, _ := doReq(t, h, "GET", "/v1/account", nil, token); code != 401 { t.Fatalf("revoked token accepted: %d", code) }
	// Logout не удаляет аккаунт или identity: тот же ключ по-прежнему позволяет войти.
	if authenticate(t, h, "legacy", kb.privEd) == "" { t.Fatal("keys lost after logout") }
}

func TestChallengeBoundToUsernameAndBounded(t *testing.T) {
	c := newChallengeStore()
	if !c.put("nonce", time.Minute, "alice") { t.Fatal("put failed") }
	if c.consume("nonce", "bob") { t.Fatal("nonce accepted for another account") }
	if !c.consume("nonce", "alice") || c.consume("nonce", "alice") { t.Fatal("nonce not single-use") }
	c.put("expired", -time.Second, "alice")
	if c.consume("expired", "alice") { t.Fatal("expired nonce accepted") }
	for i := 0; i < 4096; i++ { c.put(string(rune(i)), time.Minute, "alice") }
	if c.put("overflow", time.Minute, "alice") { t.Fatal("unbounded challenge store") }
}

func TestV2KeyTransportAndAtomicPublication(t *testing.T) {
	h := newTestServer(t)
	kb := newKeyBundle("signal_user")
	if code, _ := register(t, h, kb); code != 201 { t.Fatalf("legacy registration: %d", code) }
	...
	if code, body := doReq(t, h, "PUT", "/v1/account/keys", kb.reg, token); code != 200 { t.Fatalf("upgrade keys: %d %v", code, body) }
	code, bundle := doReq(t, h, "GET", "/v1/users/signal_user/prekeys", nil, "")
	if code != 200 || bundle["key_version"] != float64(2) || bundle["one_time_prekey_id"] != float64(99) || bundle["registration_id"] != float64(42) { t.Fatalf("bundle: %d %v", code, bundle) }
	if code, _ := doReq(t, h, "PUT", "/v1/account/keys", kb.reg, token); code != 200 { t.Fatalf("retry publish: %d", code) }
	_, exhausted := doReq(t, h, "GET", "/v1/users/signal_user/prekeys", nil, "")
	if exhausted["one_time_prekey"] != "" || exhausted["one_time_prekey_id"] != float64(0) { t.Fatalf("prekey republished: %v", exhausted) }
	kb.reg["key_bundle_id"] = "batch-2"
	kb.reg["identity_x25519"] = b64e(append([]byte{5}, bytes.Repeat([]byte{8}, 32)...))
	if code, _ := doReq(t, h, "PUT", "/v1/account/keys", kb.reg, token); code != 409 { t.Fatalf("identity rotation accepted: %d", code) }
}

func TestSendRetryAndValidation(t *testing.T) {
	h := newTestServer(t)
	alice, bob := newKeyBundle("alice"), newKeyBundle("bob")
	if code, _ := register(t, h, alice); code != 201 { t.Fatal(code) }
	code, registered := register(t, h, bob)
	if code != 201 { t.Fatal(code) }
	...
	if code != 201 { t.Fatalf("send: %d %v", code, first) }
	...
	for _, ttl := range []int64{-1, 2_592_001, 1<<62} {
		req["expires_in"] = ttl
		if code, _ := doReq(t, h, "POST", "/v1/messages", req, a); code != http.StatusBadRequest { t.Fatalf("invalid TTL %d: %d", ttl, code) }
	}
	for _, query := range []string{"?since=invalid", "?limit=0", "?limit=501"} {
		if code, _ := doReq(t, h, "GET", "/v1/messages" + query, nil, b); code != 400 { t.Fatalf("invalid cursor query: %d", code) }
	}
}
...
func TestChallengeCannotAuthenticateAnotherAccount(t *testing.T) {
	...
	if code, _ := doReq(t, h, "POST", "/v1/auth/verify", body, ""); code != 401 { t.Fatalf("cross-account challenge accepted: %d", code) }
}
