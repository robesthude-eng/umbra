...
func (p *PostgresStore) TakePrekeyBundle(ctx context.Context, username string) (*model.User, []byte, error) {
	...
	if err != nil { return nil, nil, mapErr(err) }
	var pk []byte
...
func (p *PostgresStore) UpdateKeys(ctx context.Context, userID string, keys *model.User) error {
	...
	var identity []byte
	var bundleID string
	if err := tx.QueryRow(ctx, `SELECT identity_x25519,key_bundle_id FROM users WHERE id=$1 FOR UPDATE`, userID).Scan(&identity,&bundleID); err != nil { return mapErr(err) }
	if !sameCurveIdentity(identity, keys.IdentityX25519) { return ErrConflict }
	if bundleID == keys.KeyBundleID && bundleID != "" { return tx.Commit(ctx) }
	...
	if err != nil { return err }
	if _, err = tx.Exec(ctx, `DELETE FROM one_time_prekeys WHERE user_id=$1`, userID); err != nil { return err }
	for _, pk := range keys.OneTimePrekeys {
		if _, err = tx.Exec(ctx, `INSERT INTO one_time_prekeys(user_id,prekey) VALUES ($1,$2)`, userID,pk); err != nil { return err }
	}
...
func (p *PostgresStore) SaveMediaWithQuota(ctx context.Context, media *model.Media, limit int64) error {
	...
	defer tx.Rollback(ctx)
	var userID string
	if err := tx.QueryRow(ctx, `SELECT id FROM users WHERE id=$1 FOR UPDATE`, media.OwnerID).Scan(&userID); err != nil { return mapErr(err) }
	if media.Size < 0 { return ErrQuota }
	if limit > 0 {
		var used int64
		if err := tx.QueryRow(ctx, `SELECT COALESCE(SUM(size),0) FROM media WHERE owner_id=$1`, userID).Scan(&used); err != nil { return err }
		if used > limit || media.Size > limit-used { return ErrQuota }
	}
...
func (p *PostgresStore) LockBlobs(ctx context.Context, exclusive bool) (func(), error) {
	...
	if exclusive { lock, unlock = "pg_advisory_lock", "pg_advisory_unlock" }
	// Постоянный namespace блокировки Umbra, одинаковый для server и gc.
	if _, err := conn.Exec(ctx, "SELECT "+lock+"(734219580)"); err != nil { _ = conn.Close(context.Background()); return nil, err }
	return func() {
...
func (p *PostgresStore) ListMessagesPage(ctx context.Context, userID string, since time.Time, afterID string, limit int) ([]*model.Message, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID,&m.SenderID,&recipient,&chat,&m.Ciphertext,&m.CreatedAt,&m.ExpiresAt); err != nil { return nil, err }
		if recipient != nil { m.RecipientID = *recipient }
		if chat != nil { m.ChatID = *chat }
		out=append(out,&m)
	}
	return out, rows.Err()
}
