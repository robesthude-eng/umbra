...
func (m *MemoryStore) OneTimePrekeyCount(_ context.Context, userID string) (int, error) {
    m.mu.Lock()
    defer m.mu.Unlock()
    if _, ok := m.users[userID]; !ok { return 0, ErrNotFound }
...
func sameCurveIdentity(a, b []byte) bool {
    if len(a) == 33 && a[0] == 5 { a = a[1:] }
    if len(b) == 33 && b[0] == 5 { b = b[1:] }
...
func (m *MemoryStore) TakePrekeyBundle(_ context.Context, username string) (*model.User, []byte, error) {
	...
	if !ok { return nil, nil, ErrNotFound }
...
func (m *MemoryStore) UpdateKeys(_ context.Context, userID string, keys *model.User) error {
	...
	if !sameCurveIdentity(u.IdentityX25519, keys.IdentityX25519) { return ErrConflict }
	if u.KeyBundleID == keys.KeyBundleID && keys.KeyBundleID != "" { return nil }
	...
	for i, pk := range keys.OneTimePrekeys { m.prekeys[userID][i] = append([]byte(nil), pk...) }
	return nil
}
...
func (m *MemoryStore) ListMessagesPage(ctx context.Context, userID string, since time.Time, afterID string, limit int) ([]*model.Message, error) {
	...
	sort.Slice(all, func(i, j int) bool {
		if all[i].CreatedAt.Equal(all[j].CreatedAt) { return all[i].ID < all[j].ID }
		return all[i].CreatedAt.Before(all[j].CreatedAt)
	})
	out := make([]*model.Message, 0)
	for _, msg := range all {
		if msg.CreatedAt.Equal(since) && (afterID == "" || msg.ID <= afterID) { continue }
		out = append(out, msg)
...
func (m *MemoryStore) SaveMediaWithQuota(_ context.Context, media *model.Media, limit int64) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	if _, ok := m.media[media.ID]; ok { return ErrConflict }
	if _, ok := m.users[media.OwnerID]; !ok { return ErrNotFound }
	if media.Size < 0 { return ErrQuota }
	if limit > 0 {
		var used int64
		for _, item := range m.media {
			if item.OwnerID == media.OwnerID {
				if item.Size > limit-used { return ErrQuota }
				used += item.Size
			}
		}
		if media.Size > limit-used { return ErrQuota }
	}
	cp := *media
	m.media[media.ID] = &cp
	return nil
...
func (m *MemoryStore) LockBlobs(ctx context.Context, exclusive bool) (func(), error) {
	...
	if exclusive { m.blobMu.Lock(); return m.blobMu.Unlock, nil }
	m.blobMu.RLock()
...
func (m *MemoryStore) PendingBlobDeletes(_ context.Context) ([]string, error) {
	...
	for id := range m.deletions { ids = append(ids, id); if len(ids) == 500 { break } }
	return ids, nil
}
...
func (m *MemoryStore) PurgeExpired(_ context.Context, now time.Time) error {
	...
	for _, msg := range m.messages {
		if msg.ExpiresAt == nil || msg.ExpiresAt.After(now) { kept = append(kept, msg) }
	}
	m.messages = kept
	for key, value := range m.tokens { if !value.expires.After(now) { delete(m.tokens, key) } }
	for key, receipt := range m.receipts {
		if receipt.created.Before(now.Add(-30*24*time.Hour)) { delete(m.receipts, key) }
	}
	return nil
}
